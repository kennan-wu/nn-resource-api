...
class KerasService:
    def create_neural_network(self, nn_data: CreateNNRequest):
        ...
        for i in range(1, len(nn_data.activations)):
            model.add(
                keras.layers.Dense(
                    units=nn_data.neurons_per_layer[i],
                    activation=nn_data.activations[i],
                )
            )

        loss_fn = (
            "binary_crossentropy"
            if nn_data.neurons_per_layer[-1] == 1
    ...
    def serialize_for_db(self, model: keras.Model) -> List[dict]:
        ...
        for i, layer in enumerate(model.layers):
            if isinstance(layer, keras.layers.Dense):
                weights, biases = (
                    layer.get_weights() if layer.get_weights() else ([], [])
                )
                ...
                if num_neurons <= 16:
                    ...
                    if weights.shape[0] > 16:
                        selected_weights = np.concatenate(
                            [
                                np.concatenate(
                                    [weights[:8, :8], weights[:8, -8:]], axis=1
                                ),
                                np.concatenate(
                                    [weights[-8:, :8], weights[-8:, -8:]], axis=1
                                ),
